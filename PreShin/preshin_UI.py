...
class Preshin_UI(QWidget):
    ...
    def btn_export_clicked(self):
        ...
        if self.lbl_lbl.text() != '' and self.lbl_pre.text() != '':
            ...
            if self.edt_xlsx_name.text() != '':    # 파일명 입력 했을때
                loc_xlsx = QFileDialog.getExistingDirectory(self, self.tr("Open file"), 'C:/woo_project/AI/root',
                                                            QFileDialog.ShowDirsOnly)
                file = os.listdir(loc_xlsx)
                if self.edt_xlsx_name.text()+'.xlsx' not in file:    # 동일한 파일명이 없을때
                    if loc_xlsx != '':    # 폴더 선택 했을때
                        df_sheet = pd.DataFrame()
                        self.set_pre_lbl()  # id 정렬
                        wb = openpyxl.Workbook()
                        self.new_xlsx = loc_xlsx + f'/{self.edt_xlsx_name.text()}.xlsx'
    ...
    def sheet2_setting(self):
        ...
        ws.cell(row=4, column=2).fill = self.yellow_color
        ...
        for i in range(len(group_key)):
            ...
            for j in range(len(group_value)):
                num=0
                result=0
                for row in range(b,b+len(group_value[i])):
                    data = ws.cell(row=row, column=2).value
        ...
        c = 5
        n = 0
        for h in range(len(group_value)):
            ws.insert_rows(c+n)
            n += 1
            c += len(group_value[i]) + 1
        ...
        self.matploblib()
    def matploblib(self):
        df = pd.read_excel(self.new_xlsx,sheet_name='Sheet2', header=2)
